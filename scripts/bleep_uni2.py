...
class CFG:
    ...
    ckpt_dir = "/rsrch5/home/plm/phacosta/models/fine_tuned/UNI2/bleep_style_v2"  # outputs
...
for epoch in range(1, CFG.epochs + 1):
    ...
    if avg < best:
        best = avg
        # torch.save({"cfg": CFG.__dict__, "epoch": epoch, "model": model.state_dict()},
        #            f"{CFG.ckpt_dir}/best.pth")
        
        torch.save(
            {
                "cfg":   cfg_to_dict(CFG),   # <- now pickle-able
                "epoch": epoch,
                "model": model.state_dict()
            },
            f"{CFG.ckpt_dir}/best.pth"
