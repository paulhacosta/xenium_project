...
#%%
task = "fine_tune" # pretrained or fine_tune
...
if task == "pretrained":
    cells_df = pd.read_parquet(os.path.join( data_path, "cells.parquet"))  # or pd.read_parquet("path_to/cells.parquet")
...
sc.pp.filter_cells(adata, min_genes=10)  # Keep cells with at least 10 genes
...
if task == "pretrained":
    ...
    if ground_truth == "aitil":
        ...
        print("Saved file to:", os.path.join(data_path, "preprocessed", task+f"_{ground_truth}_v2", f"processed_xenium_data_fine_tune_{ground_truth}_v2.h5ad"))
