...
level = 0
cluster_method = "kmeans" # kmeans or leiden
...
if platform == "xenium":
    ...
    data_path = "/rsrch9/home/plm/idso_fa1_pathology/TIER1/paul-xenium/public_data/10x_genomics/Visium_HD_Human_Lung_Cancer_post_Xenium_Prime_5k_Experiment2/binned_outputs/square_002um/preprocessed/bin2cell/corrected_cells_matched_preprocessed_refined_v2.h5ad"
...
#%% Cluster features - custom defined for morphology embeddings alone
if cluster_method == "kmeans":
    ...
    scatter = sns.scatterplot(data=morph_embeddings, x='x', y='y', hue='Cluster_reduced', palette='Set1', alpha=0.7, s=10)
    ...
    plt.axis("off")
    fig_save_dir = morph_embedding_csv.replace(".csv","clusters.png")
...
if cluster_method == "leiden":
    ...
    ad.obs['x'] = spatial_coords.loc[ad.obs_names, 'x']
    ad.obs['y'] = spatial_coords.loc[ad.obs_names, 'y']

    n_colors = 20
    cmap = plt.cm.get_cmap("tab20", n_colors)   # fresh copy each call

    plt.figure(figsize=(8, 6))
    plt.scatter(
        ad.obs['x'], ad.obs['y'],
        c=ad.obs['leiden'].astype(int),
        cmap=cmap, s=1, vmin=0, vmax=n_colors-1   # fixed mapping
    )
    
    plt.gca().invert_yaxis()
    plt.axis('equal')
    plt.title("Spatial clusters – UNI2 Pretrained")
